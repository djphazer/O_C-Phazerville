...
Next, the Compressor kicks in when signals exceed its Threshold, and reduces level with somewhat slower attack and release.

Then you get to boost everything with the make-up gain, if you want; "auto" makes some kind of guess for you.

Finally, the Limiter tries its hardest to clamp the post-Gain signal with the shortest possible attack/release. Transients that sneak through the Limiter will be hard-clipped... ;)
